...
export function HomeDashboard({
  ...
  const availableTabs = useMemo(() => {
    const tabs: TabKey[] = [];
    if (canViewDocumentInHand) tabs.push("document_in_hand");
    if (canViewInbound) tabs.push("inbound");
    if (canViewOutbound) tabs.push("outbound");
    if (canViewHistory) tabs.push("history");
    return tabs;
  ...
  const handleTransfer = async () => {
    if (!canTransfer) {
      alert("You do not have permission to transfer documents.");
      return;
    }
  ...
  const handleConfirmReceive = async () => {
    if (!canReceive) {
      alert("You do not have permission to receive bundles.");
      return;
    }
      ...
      {availableTabs.length > 0 ? (
        <>
          ...
          <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-xs">
                    ...
                    <table className="w-full text-left text-sm text-slate-700">
                      <thead className="bg-slate-50 text-xs font-bold text-slate-700 tracking-wider border-b border-slate-200">
                        <tr>
                          ...
                          {canReceive && <th className="p-4 text-right">Actions</th>}
                        </tr>
                      </thead>
            ...
            {activeTab === "outbound" && canViewOutbound && (
              <div>
                    ...
                    <table className="w-full text-left text-sm text-slate-700">
                      <thead className="bg-slate-50 text-xs font-bold text-slate-700 tracking-wider border-b border-slate-200">
                        <tr>
                          ...
                          <th className="p-4 text-center">Finished Days</th>
                          {canRetrieve && <th className="p-4 text-right">Actions</th>}
                        </tr>
                      </thead>
                      <tbody className="divide-y divide-slate-200 bg-white">
                        {outboundBundles.map((bundle: any, index: number) => {
                          ...
                          const canPerformRetrieve = !isFullyReceived && !isRetrieved;
        ...
        <EmptyState
          icon={Package}
          title="No Home Workflow Access"
          description="You do not have permission to view any sections under Home Workflow. Please contact your Super Admin."
        />
      )}
