import { getBmReportStats, listBmOutward } from "@/features/bm-report/server/bm-report.service";
...
export async function GET() {
  ...
  try {
    ...
    const [items, stats] = await Promise.all([
      listBmOutward(ownerAdminId, officeLocationName),
      getBmReportStats(ownerAdminId, officeLocationName),
    ]);

    return jsonOk({ items, stats });
  } catch (error) {
    console.error("Failed to fetch BM outward", error);
    return jsonError("Unable to fetch BM outward.", 500);
  }
}
